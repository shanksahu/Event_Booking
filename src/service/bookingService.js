...
class BookingService {
  async holdSeats(data, next) {
    ...
    try {
      ...
      const seats = await seatAvailabilityModel.find({
        _id: { $in: seatIds },
        eventId,
      });
      if (!seats.length) {
      ...
      for (const seat of seats) {
        if (seat.isBooked || seat.isHold) {
          return responseStructure(
            400,
            false,
            `Seat number ${seat.seatNumber} is not available`
          );
        }
      }
      ...
      const initiateBooking = await bookingModel.create([bookingData], {
        session,
      });
      ...
      session.endSession();
      return responseStructure(200, true, 'Seats held successfully', {
        holdId,
      });
    } catch (error) {
  ...
  async confirmSeats(data, next) {
    ...
    try {
      ...
      session.startTransaction();
      transactionStarted = true;
      let bookingDetails = await bookingModel.findOneAndUpdate(
        { holdId },
        bookingData,
        { session, new: true }
      );
      ...
      return responseStructure(200, true, 'Seats booked successfully', {
        bookingId: bookingDetails._id,
      });
    } catch (error) {
      if (transactionStarted) {
  ...
  async getBooking(data, next) {
    try {
      ...
      let bookingDetails = await bookingModel
        .findById(bookingId, project)
        .populate('seatIds eventId', 'seatNumber eventName eventDate location');
      if (!bookingDetails) {
        return responseStructure(
          409,
          false,
      ...
      return responseStructure(200, true, 'Booking details fetched', {
        bookingDetails,
      });
    } catch (error) {
      next(error);
